...
router.get('/loans', async (req, res) => {
  try {
    ...
    // Enrich loans with approver names
    ...
    res.render('member/loans', { user: m.toJSON(), group: group.toJSON(), loans: enriched, repayments, balance, eligibleAmount, hasActiveLoan, hasPendingLoan, activeLoan: activeLoan?.toJSON()||null, query: req.query });
  } catch (err) { console.error(err); res.render('error', { message: 'Error', user: req.user }); }
});
...
router.post('/loans/apply', async (req, res) => {
  try {
    ...
    // Block if already has active or pending loan (unless top-up)
    const activeLoan = await Loan.findOne({ where: { memberId: m.id, status: 'active' } });
    ...
    // Top-up: must have active loan
    if (loanType === 'top_up' && !activeLoan) return res.redirect('/member/loans?error=no_active_loan_for_topup');

    // Check eligibility
    const limit = loanType === 'emergency' ? balance * 1 : balance * 3;
    if (parsedAmount > limit) return res.redirect('/member/loans?error=exceeds_limit');
    ...
    // Notify credit officer first (stage 1), or admin if no credit officer
