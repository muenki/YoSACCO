...
async function seed() {
  try {
    ...
    const creditOfficer = await User.create({ name:'Peter Ssemakula',  email:'credit@kteachers.coop',        password:hash('Admin@2025'), role:'credit_officer', groupId:grp1.id, memberId:'KTS-0001', phone:'+256700111111', active:true });
    const treasurer     = await User.create({ name:'Sarah Nambi',      email:'treasurer@kteachers.coop',     password:hash('Admin@2025'), role:'treasurer',      groupId:grp1.id, memberId:'KTS-0002', phone:'+256700222222', active:true });
    const chairperson   = await User.create({ name:'John Musoke',      email:'chair@kteachers.coop',         password:hash('Admin@2025'), role:'chairperson',    groupId:grp1.id, memberId:'KTS-0003', phone:'+256700333333', active:true });
